...
data "template_file" "ansible_inventory" {
  ...
  vars = {
    hosts = join(
      "",
      [
        data.template_file.ansible_aws_host.rendered,
        data.template_file.ansible_gcp_host.rendered,
      ],
    )
    vpns = join(
      "",
      [
        data.template_file.ansible_aws_vpn.rendered,
        data.template_file.ansible_gcp_vpn.rendered,
      ],
    )
